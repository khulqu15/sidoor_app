...
onMounted(async() => {
    ...
    const labeledFaceDescriptors = await Promise.all(
        labels.map(async label => {
            ...
            const fullFaceDescriptions = await faceapi.detectSingleFace(img).withFaceLandmarks()
            if(!fullFaceDescriptions) throw new Error(`no faces detected for ${label}`)
            const faceDescriptors = fullFaceDescriptions ? [Float32Array.from(fullFaceDescriptions.descriptor)] : []
