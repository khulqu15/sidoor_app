        ...
        <ion-content>
            <div class="min-h-screen bg-base-100 relative">
                <div style="position: relative; width: 500px" class="margin">
                    <video @play="onPlay()" id="inputVideo" width="720" height="560" autoplay muted></video>
...
function onPlay() {
    const face_canvas = faceapi.createCanvasFromMedia(videoEl)
    document.body.append(face_canvas)
    const displaySize = {width: videoEl.width, height: videoEl.height}
    setInterval(async () => {
        const detetions = await faceapi.detectAllFaces(videoEl, new faceapi.TinyFaceDetectorOptions()).withFaceLandmarks().withFaceExpressions()
        const resizedDetections = faceapi.resizeResults(detetions, displaySize)
