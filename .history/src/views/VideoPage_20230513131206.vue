...
async function run() {
    ...
    faceapi.draw.drawFaceLandmarks('overlay', mtcnnResults.map(res => res.detection), { lineWidth: 4, color: 'red' })


    
    let fullFaceDescriptions = await faceapi.detectAllFaces(videoEl).withFaceLandmarks().withFaceDescriptors()
    fullFaceDescriptions = faceapi.resizeResults(fullFaceDescriptions, videoEl)
    faceapi.matchDimensions(canvas, videoEl)
