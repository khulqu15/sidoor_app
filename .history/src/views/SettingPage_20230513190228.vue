<template>
    <ion-page>
        <ion-content>
            <div class="min-h-screen w-full relative overflow-hidden">
                <div class="absolute w-full h-full z-20 bg-black/70"></div>
                <img src="/assets/wallpaper.jpg" class="absolute h-full w-full object-cover object-center" alt="wallpaper">
                <div class="grid text-white relative z-30 grid-cols-1 items-center min-h-screen justify-items-center p-8">
                    <div class="space-y-6 max-w-md">
                        <h3 class="mb-8 font-bold">Setting Parameter</h3>
                        <div class="space-y-3">
                            <label class="text-white" for="timer">Timer Auto Close Door (Second)</label>
                            <input v-model="timer" @keyup="saveData()" type="number" min="10" minlength="10" class="input w-full bg-black/50 text-white" />
                        </div>
                        <div class="space-y-3">
                            <label class="text-white" for="timer">Pin lock value</label>
                            <input v-model="pin" @keyup="saveData()" type="number" min="10" minlength="10" class="input w-full bg-black/50 text-white" />
                        </div>
                        <div class="space-y-3">
                            <label class="text-white" for="timer">Pattern lock value</label>
                            <input v-model="pattern" @keyup="saveData()" type="number" class="input w-full bg-black/50 text-white" />
                        </div>
                        <div class="grid grid-cols-2 gap-x-4">
                            <div>
                                <button @click="recordData('finger')" class="btn bg-primary/20 border-2 border-primary text-white w-full">
                                    <Icon icon="heroicons:finger-print-20-solid" class="text-2xl mr-3"/>
                                    New Finger
                                </button>
                            </div>
                            <div>
                                <button @click="recordData('rfid')" class="btn bg-primary/20 border-2 border-primary text-white w-full">
                                    <Icon icon="tabler:nfc" class="text-2xl mr-3"/>
                                    New RFID
                                </button>
                            </div>
                        </div>
                        <div>
                            <button @click="saveDataAndExit()" class="btn bg-primary w-full">Save Data</button>
                        </div>
                    </div>
                </div>
                <input type="checkbox" v-model="modal_checkbox" id="modal-action" class="modal-toggle" />
                <div class="modal text-white">
                    <div class="modal-box bg-black/60 backdrop-blur-lg w-11/12 max-w-lg text-center">
                        <h3 class="font-bold text-lg">Record Data {{ mode == 'finger' ? 'FingerPrint' : 'RFID Card' }}</h3>
                        <Icon v-if="mode == 'finger'" icon="heroicons:finger-print-20-solid" class="text-6xl mt-3 mr-3 text-primary inline-block"/>
                        <Icon v-else icon="tabler:nfc" class="text-6xl mt-3 mr-3 text-primary inline-block"/>
                        <p class="py-4">{{ mode == 'finger' ? 'Please place your finger on the fingerprint scanner.' : 'Please attach your card to the RFID / NFC reader' }}</p>
                        <div class="modal-action">
                            <button @click="closeModal()" class="btn w-full">Cancel</button>
                        </div>
                    </div>
                </div>
                <input type="checkbox" v-model="modal_success" id="modal-success" class="modal-toggle" />
                <div class="modal text-white">
                    <div class="modal-box bg-black/60 backdrop-blur-lg w-11/12 max-w-lg text-center">
                        <Icon icon="material-symbols:check-circle-rounded" class="text-6xl mt-3 mr-3 text-primary inline-block"/>
                        <h3 class="font-bold text-lg">Record Data Success</h3>
                        <div class="modal-success">
                            <button @click="closeModal()" class="btn w-full">Close</button>
                        </div>
                    </div>
                </div>
                <input type="checkbox" v-model="error.show" id="modal-action" class="modal-toggle" />
                <div class="modal">
                    <div class="modal-box text-white bg-black/60 backdrop-blur-lg w-11/12 max-w-lg text-center">
                        <Icon v-if="mode == 'finger'" icon="heroicons:finger-print-20-solid" class="text-6xl mt-3 mr-3 text-primary inline-block"/>
                        <Icon v-else icon="iconamoon:sign-times-circle-fill" class="text-6xl mt-3 mr-3 text-red-500 inline-block"/>
                        <h3 class="font-bold text-lg">{{ error.message }}</h3>
                        <div class="modal-action">
                            <button @click="closeModalError()" class="btn w-full">Close</button>
                        </div>
                    </div>
                </div>
            </div>
        </ion-content>
    </ion-page>
</template>

<script lang="ts">
import { IonContent, IonPage } from '@ionic/vue'
import { defineComponent } from 'vue'
import { Icon } from '@iconify/vue'
import { ref as firebaseRef, set, onValue } from "firebase/database"
import { db } from "@/firebase"
export default defineComponent({
    data() {
        return {
            modal_checkbox: false,
            modal_success: true,
            mode: 'finger',
            timer: 10,
            pin: 11223, 
            pattern: 12369,
            listen_success: false,
            error: {
                message: "",
                show: false,
            }
        }
    },
    components: {
        IonContent,
        IonPage,
        Icon
    },
    created() {
        this.fillRTDBValue()
    },
    methods: {
        closeModalError() {
            this.error.show = false
            const errorMessageValue = firebaseRef(db, 'error/show')
            set(errorMessageValue, false)
        },
        fillRTDBValue() {
            const timerValue = firebaseRef(db, 'timer')
            onValue(timerValue, (snapshot) => {
                const data = snapshot.val()
                this.timer = parseInt(data) / 1000
            })
            const errorMessageValue = firebaseRef(db, 'error/message')
            onValue(errorMessageValue, (snapshot) => {
                const data = snapshot.val()
                this.error.message = data
            })
            const errorValue = firebaseRef(db, 'error/show')
            onValue(errorValue, (snapshot) => {
                const data = snapshot.val()
                this.error.show = data
            })
            const pinValue = firebaseRef(db, 'app/pin')
            onValue(pinValue, (snapshot) => {
                const data = snapshot.val()
                this.pin = parseInt(data)
            })

            const patternValue = firebaseRef(db, 'app/pattern')
            onValue(patternValue, (snapshot) => {
                const data = snapshot.val()
                const formatter = data.replace(/,/g, '')
                this.pattern = parseInt(formatter)
            })

            const listenSuccessValue = firebaseRef(db, 'success')
            onValue(listenSuccessValue, (snapshot) => {
                const data = snapshot.val()
                if(data) {
                    this.modal_success = true
                }
            })
        },
        recordData(method: string) {
            this.mode = method
            this.modal_checkbox = true
            if(this.mode == 'finger') {
                const saveFingerRef = firebaseRef(db, 'save_finger')
                set(saveFingerRef, true)
            } else if(this.mode == 'rfid') {
                const saveRFIDRef = firebaseRef(db, 'save_rfid')
                set(saveRFIDRef, true)
            }
        },
        closeModal() {
            this.modal_checkbox = false
            this.modal_success = false
            const saveFingerRef = firebaseRef(db, 'save_finger')
            set(saveFingerRef, false)
            const saveRFIDRef = firebaseRef(db, 'save_rfid')
            set(saveRFIDRef, false)
            const listenSuccessValue = firebaseRef(db, 'success')
            set(listenSuccessValue, false)
        },
        saveData() {
            const timerRef = firebaseRef(db, 'timer')
            if(this.timer < 10) set(timerRef, 10000)
            else set(timerRef, (this.timer * 1000))

            const pinRef = firebaseRef(db, 'app/pin')
            set(pinRef, this.pin)

            const pattern_digits = this.pattern.toString().split('')
            const pattern = pattern_digits.join(',')
            const patternRef = firebaseRef(db, 'app/pattern')
            set(patternRef, pattern) 
        },
        saveDataAndExit() {
            this.closeModal()
            this.saveData()
            this.$router.push({name: 'Home'})
        }
    }
})
</script>